...
def extract_unique_brand_sku_pairs(input_file_path: str) -> list[tuple[str, str]]:
    """
    Extract unique brand-SKU pairs from the input file.

    Args:
        input_file_path: Path to the input file
    Returns:
        List of tuples containing brand and article information

    Example:
        >>> pairs = extract_unique_brand_sku_pairs('input_file.xlsx')
        >>> print(pairs[:3])
        [('hyundai/kia/mobis', '223112e100'),
         ('hyundai/kia/mobis', '282852f000'),
         ('ssangyong', '000007008106')]
    """
    df = pd.read_csv(input_file_path) if input_file_path.lower().endswith(".csv") else pd.read_excel(input_file_path)

    # Normalize columns
    df["Бренд"] = df["Бренд"].astype(str).str.strip().str.lower()
    df["Артикул"] = df["Артикул"].astype(str).str.strip().str.lower()

    # Deduplicate
    unique_df = df.drop_duplicates(subset=["Бренд", "Артикул"])

    # Convert to list of tuples
    brand_sku_pairs = list(unique_df[["Бренд", "Артикул"]].itertuples(index=False, name=None))
    return brand_sku_pairs

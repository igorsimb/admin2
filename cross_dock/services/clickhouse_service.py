...
def query_supplier_data_mv(
    ...
    Args:
        ...
        days_lookback: Number of days to look back for supplier data (default: 3)
    ...
    try:
        ...
        FROM (
            SELECT
                ...
                brand_lower,
                sku_lower,
                ROW_NUMBER() OVER (PARTITION BY brand_lower, sku_lower, supplier_name ORDER BY update_date DESC) AS rn,
            ...
            WHERE
                ...
                (brand_lower, sku_lower) IN %(brand_sku_pairs)s AND
                update_date >= now() - interval %(days_lookback)s day
        )
        WHERE rn = 1 AND rank <= 3
